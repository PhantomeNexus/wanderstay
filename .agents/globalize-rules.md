<!-- Generated by globalize-guide. Template: next-intl v1. Regenerate rather than hand-editing. -->
...
**This project:**
- Catalogs: `messages/{locale}.json` — add every new message here in the same change that uses it.
- Source locale `en` (write every source string in it); target locales `de, fr, es, it, pt, ar`.
...
| `Link`, `redirect`, `useRouter`, `usePathname`, `getPathname` | `@/i18n/navigation` (locale-aware wrappers from `createNavigation`) |
...
// components/Nav.tsx — import { NAV } from '@/lib/nav'
...
- Leave `<NextIntlClientProvider>` in `app/[locale]/layout.tsx` without an explicit `messages` prop unless you need to filter; next-intl then serialises only what the client tree references. Passing every message ships the whole catalog to every page.

**Client components** (`'use client'`):
- Use `useTranslations()` and `useFormatter()` (synchronous hooks), inside the `<NextIntlClientProvider>` tree.
- Take `Link`, `useRouter`, `usePathname` and `redirect` from `@/i18n/navigation`, never from `next/link` or `next/navigation`.
...
**Named formats** — define reusable presets in `src/i18n/request.ts` (part of the object `getRequestConfig` returns), then use them by name: `format.dateTime(date, 'short')`, `format.number(value, 'precise')`.
...
`@/i18n/navigation` calls `createNavigation(routing)` and re-exports `Link`, `redirect`, `useRouter`, `usePathname`, `getPathname`. Use those, never `next/link` or `next/navigation` — Next's own `Link` is locale-blind and emits `/about` whatever the active locale is.
